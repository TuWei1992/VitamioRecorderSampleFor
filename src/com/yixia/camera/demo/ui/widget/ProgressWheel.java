...
public class ProgressWheel extends View {
	...
	private Handler spinHandler = new Handler() {
		/**
		 * This is the code that will increment the progress variable and so spin
		 * the wheel
		 */
		@Override
	...
	private void setupPaints() {
		...
		textPaint.setShadowLayer(1, 1, 1, Color.parseColor("#44000000"));
	}
	...
	private void parseAttributes(TypedArray a) {
		...
		progress = a.getInteger(R.styleable.ProgressWheel_rimProgress, progress);
		if (progress < 0) {
			progress = 0;
		}

		isSpinning = !a.getBoolean(R.styleable.ProgressWheel_incrementMode, false);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawCircle((circleBounds.width() / 2) + rimWidth + paddingLeft, (circleBounds.height() / 2) + rimWidth + paddingTop, circleRadius, circlePaint);
		//Draw the text (attempts to center it horizontally and vertically)
		int offsetNum = 0;
		for (String s : splitText) {
			float offset = textPaint.measureText(s) / 2;
			//			canvas.drawText(s, this.getWidth() / 2 - offset, this.getHeight() / 2 + (textSize * (offsetNum)) - ((splitText.length - 1) * (textSize / 2)), textPaint);
			canvas.drawText(s, this.getWidth() / 2 - offset, this.getHeight() / 2 + textPaint.getFontMetrics().bottom, textPaint);
			offsetNum++;
		}
	}
	...
	public void setProgressEx(int progress) {
		isSpinning = false;
		if (progress < 0)
			progress = 0;
